...
export function CommunityFeed({ userLocation }: CommunityFeedProps) {
  ...
  const activePincode = activeLocation?.pincode || '600004'; // Default to Mylapore
      ...
      <div className="px-6 py-4">
        <div className="grid grid-cols-4 gap-3 mb-4">
          ...
          <Button
            variant="outline"
            className="flex-col h-auto py-3 border-orange-200 hover:bg-orange-50"
            onClick={() => navigate('/chennai-gethu')}
          >
            <IllustratedIcon src={ChennaiIcons.beach} alt="Chennai Gethu" size="sm" className="mb-1" />
            <span className="text-xs text-[11px]">Chennai Gethu</span>
          </Button>
        </div>

        {/* Create Post Dialog (Only show if not in specialized modes) */}
        {!activeFilter && (
